...
//     .option(
//         '-D, --development',
//         'development mode, does not generate code for production',
//         false,
//     )
//     .parse([...process.argv, './'])
//     .opts<{
...
//     if (!QUIET)
//         console.log(
//             `${colors.yellow}Warning: ${colors.cyan}--no-typing ${colors.reset}and ${colors.cyan}--strict ${colors.reset}are incompatible, --strict will be ignored`,
//         )
// }
// if (LANG === 'ts' && NO_TYPING && MAPPING && !DEVELOPMENT) {
//     if (!QUIET)
//         console.log(
//             `${colors.yellow}Warning: ${colors.cyan}--no-typing ${colors.reset}and ${colors.cyan}--mapping ${colors.reset}are incompatible, --strict will be ignored`,
//         )
// }
...
//     if (!QUIET)
//         console.log(
//             `${colors.red}Error: ${colors.cyan}${PATH} ${colors.reset}is empty`,
//         )
// }
...
//             join(process.cwd(), PATH, dirent.name),
//             { with: { type: 'json' } }
//         )
//         JSONS.push(json)
...
//     } catch (error) {
//         if (
//             (error as Error).message.includes('Unexpected end of JSON input') &&
//             !QUIET
//         )
//             console.error(
//                 `${colors.yellow}Warning: ${colors.cyan}${dirent.name} ${colors.reset}is empty`,
//             )
//         else if (
...
//         )
//             console.error(
//                 `${colors.red}Error: ${colors.cyan}${dirent.name} ${colors.reset}is not a valid JSON`,
//             )
//         else if (
...
// const intlForList = new Intl.ListFormat('en', {
//     style: 'long',
//     type: 'conjunction',
// })
// if (!QUIET)
//     console.log(
//         `${colors.blue}Info: ${colors.cyan}${intlForList.format(
//             objectsNames,
//         )} ${colors.reset}files loaded`,
//     )
...
//     writeFileSync(
//         outputFileName,
//         JSON.stringify(outJSON, null, +INDENT),
//         'utf-8',
//     )
//     if (!QUIET)
//         console.log(
//             `${colors.green}Success: ${colors.cyan}${outputFileName}${colors.reset} created`,
//         )
//     process.exit(0)
// }
...
//             out += `${' '.repeat(indentSpace)}"${key}": "${[...path, key].join(
//                 '.',
//             )}", ${
//                 mustaches
//                     ? `/** @description "${getMustache(key, objs)}" */`
//                     : ''
//             }\n`
//         } else if (
//             typeof value === 'object' &&
...
//             !Array.isArray(value)
//         ) {
//             out += `${' '.repeat(indentSpace)}"${key}": ${createObject(
//                 obj[key],
//                 objs.map(o => o[key]).filter(Boolean),
//                 [...path, key],
//                 indentSpace + +INDENT,
//                 true,
//             )}`
...
//     }
//     return (
//         out + `${' '.repeat(indentSpace - +INDENT)}}${recursive ? ',' : ''}\n`
//     )
// }
...
//             const [existsInAll, allAreString] = existsAndIsSameType(
//                 key,
//                 objs,
//                 'string',
//             )
...
//                 if (!QUIET)
//                     console.error(
//                         `${colors.red}Error: ${colors.cyan}${path
//                             .concat(key)
//                             .join('.')} ${
//                             colors.reset
//                         }is not a same type in all files`,
//                     )
//             } // is a same type in all files
//             else
//                 out += `${' '.repeat(indentSpace)}"${key}"${
//                     !existsInAll && STRICT ? '?' : ''
...
//         ) {
//             // check if exists in other files
//             const [existsInAll, allAreObjects] = existsAndIsSameType(
//                 key,
//                 objs,
//                 'object',
//             )
...
// function existsAndIsSameType(
//     key: string,
//     objs: Array<Record<string, any>>,
//     typeToCheck: typesString,
// ): [boolean, boolean] {
//     const existsInAll = objs.every(o => typeof o[key] !== 'undefined')
//     const allAreString = objs.every(o =>
//         typeToCheck === 'object'
//             ? typeof o[key] === 'object' &&
//               o[key] !== null &&
//               !Array.isArray(o[key])
...
//     } else if (DEVELOPMENT)
//         writeFileSync(
//             join(import.meta.dirname, 'LocalesMap.d.ts'),
//             `${createMapping()}\n\nexport {}`,
//             'utf-8',
//         )
//     else
//         writeFileSync(
//             outputFileName,
//             `type Locales = ${outType}\n\nconst keys: Locales = ${outObj}${
//                 SEMI ? ';' : ''
//             }\n\nexport default keys ${MAPPING ? createMapping() : ''}`,
//             'utf-8',
//         )
//     if (!QUIET)
//         console.log(
//             `${colors.green}Success: ${colors.cyan}${
//                 DEVELOPMENT ? '' : outputFileName
//             }${colors.reset}${DEVELOPMENT ? 'types' : ''} ${
//                 DEVELOPMENT ? 'loaded' : 'created'
//             }`,
//         )
//     process.exit(0)
// }
